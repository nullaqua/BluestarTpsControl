...
public final class Main
{
    ...
    static
    {
        ...
        try
        {
            Main.class.getClassLoader().loadClass("me.lanzhi.bluestartpscontrol.BluestarTpsControlAPI");
        }
        ...
        public byte[] transform(final ClassLoader loader,String className,final Class<?> classBeingRedefined,final ProtectionDomain protectionDomain,final byte[] classfileBuffer)
        {
            ...
            if (className.equals("org.spigotmc.TicksPerSecondCommand"))
            {
                try
                {
                    ...
                    if (method.getReturnType().getName().endsWith("String"))
                    {
                        ...
                        method.setBody(
                        "{ return ($1>21.0D?"+aqua+":$1>18.0D?"+green+":$1>16.0D?"+yellow+":"+red+").toString() +"+
                    ...
                    else
                    {
                        ...
                        method.setBody(
                            "{return "+component+".text(\"\"+((double)Math.round($1*100.0D)/100.0D),"+
                            "($1>21.0D?"+aqua+":$1>18.0D?"+green+":$1>16.0D?"+yellow+":"+red+"));}");
                    }
            ...
            if (className.equals("net.minecraft.server.MinecraftServer"))
            {
                ...
                try
                {
                    ...
                    String insert="{ $0."+field.getKey()+"+=BluestarMSPT-50L;$0."+field.getValue()+"+=BluestarMSPT-50L; }";
                    System.err.println(insert);
                    clazz.getDeclaredMethod(methodName).insertBefore(insert);
                    clazz.addMethod(CtNewMethod.make("public static boolean setmspt(long mspt){ if(mspt>0L){BluestarMSPT=mspt;return true;}else return false; }",clazz));
                    ...
                    //CtMethod method=CtNewMethod.make("public static me.lanzhi.bluestartpscontrol.BluestarTpsControlAPI getAPI();",api);
